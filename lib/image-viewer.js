...
(() => {
  ...
  //
  // MUST stay in lock-step with long-image-viewer.js RATIO_THRESHOLD.
  // When the two disagree (e.g. LIV at 3.0 but here at 2.0, the v1.6.11
  // regression), images in the (LIV_low, IV_low] band lose both paths:
  // image-viewer bails thinking LIV will take over, LIV refuses thinking
  // it's not tall enough → normal zoom dies (#44).
  // Also: `hasTallImage` scans EVERY img in the dialog. One medium-tall
  // sibling in a multi-image tweet would otherwise kill zoom on the
  // wide images too. Keeping the thresholds aligned avoids that.
  const LIV_RATIO_THRESHOLD = 3.0;
